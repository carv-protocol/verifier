...
func wireApp(bootstrap *conf.Bootstrap, logger log.Logger, helper *log.Helper) (*kratos.App, func(), error) {
	...
	transactionUsecase := biz.NewTransactionUsecase(transactionRepo, helper)
	transactionService := service.NewTransactionService(transactionUsecase, logger)
	grpcServer := server.NewGRPCServer(bootstrap, verifierService, transactionService, logger)
	httpServer := server.NewHTTPServer(bootstrap, verifierService, transactionService, logger)
	workerServer := worker.NewWorkerServer(bootstrap, dataData, helper, verifierRepo, transactionRepo)
