...
func parseTCB(sgxExtMap map[string]asn1.RawValue, compSVNOIDs []asn1.ObjectIdentifier, pceSVNOID asn1.ObjectIdentifier) (TCB, error) {
	...
	if err != nil {
		return tcb, errors.New(fmt.Errorf("failed to unmarshal ASN.1 sequence: %w", err).Error())
	}
	...
	if err != nil {
		return tcb, errors.New(fmt.Errorf("error parsing PCE SVN: %w", err).Error())
	}
	tcb.PceSVN = pceSVN
	...
	for i := 0; i < len(compSVNArray); i++ {
		if j > len(tcb.CompSVNArray) {
			return tcb, errors.New(fmt.Errorf("error parsing Comp SVN: %w", err).Error())
		}
		if compSVNArray[i] > 0 {
